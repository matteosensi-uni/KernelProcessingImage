...
TEST(ImageTest, ReadingEmptyFile){
    EXPECT_THROW(Image<int>("../../test/fileVuoto.pgm"), std::logic_error); //empty file
    EXPECT_NO_THROW(Image<int>("../../test/validFile.ppm")); //valid file
}

TEST(ImageTest, InvalidPath){
    EXPECT_THROW(Image<int>("pippo"), std::logic_error); //invalid path
